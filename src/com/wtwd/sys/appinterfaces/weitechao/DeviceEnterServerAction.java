...
public class DeviceEnterServerAction extends BaseAction {

	Log logger = LogFactory.getLog(DeviceEnterServerAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			int devp = Integer.parseInt(Tools.getDevPort());
			int httpp = Integer.parseInt(Tools.getHttpPort());
			
			String ip = "";
			if (listP.size() > 0) {
				// 服务器的ip
				ip = GetIp.getRealIp();
				int port = devp;	//6688;	//6690
				int httpPort=httpp;	//8080;	//8087
				po.setCondition("device_imei ='" + imei + "'");
				po.setCountNum(ip);
				po.setPort(port);
				po.sethPort(httpPort);
				po.setShutdown(sf.format(new Date()));
				...
				po.sethPort(httpPort);
				ServiceBean.getInstance().getPhoneInfoFacade()
						.insertDeviceServiceActiveInfo(po);

				json.put(Constant.RESULTCODE, 1);
			}
			json.put("port", 6690);   //6690
			json.put("ip", ip);
			json.put("imei", imei);

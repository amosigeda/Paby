...
public class IPSeeker {
     ...
     */
	//纯真IP数据库名
	private String IP_FILE="D:/QQWry.Dat";
	//保存的文件夹
    ...
    public IPSeeker(String fileName,String dir)  {
				...
				if(files[i].isFile()) {
					if(files[i].getName().toLowerCase().equals(filename)) {
						try {
							...
							LogFactorys.log("IP地址信息文件没有找到，IP显示功能将无法使用",Level.ERROR,e1);
							ipFile = null;
						}
						break;
					}
		...
		if(ipFile != null) {
			try {
				...
				LogFactorys.log("IP地址信息文件格式有错误，IP显示功能将无法使用",Level.ERROR,e);
				ipFile = null;
			}			
		}
	}
	...
	private long locateIP(byte[] ip) {
		...
		for(long i = ipBegin, j = ipEnd; i < j; ) {
			...
			readIP(m, b4);
			r = compareIP(ip, b4);
			// log.debug(Utilss.getIpStringFromBytes(b));
			if(r > 0)
				i = m;
			else if(r < 0) {
				if(m == j) {

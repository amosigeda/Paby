 ...
 * @version V2.0
 */
package com.wtwd.common.charset;
...
public class CharsetDecoderII extends CumulativeProtocolDecoder {
	...
	protected boolean doDecode(IoSession session, IoBuffer in,
		...
		logger.info("in CharsetDecoderII dodecode session is ==========================="
				+ session);
		int inRemain = in.remaining();
		logger.info("in CharsetDecodeII dodecode inRemain is =================="
				+ inRemain);
		if (inRemain < 4)
			return false;
		...
		System.out.println("是否="+in.hasRemaining());
		...
		System.out.println("test--");
			...
			if (test == b) { // 匹配第matchCount位换行符成功
				...
				System.out.println("delimBuf.limit()-------"+delimBuf.limit());
				if (matchCount == delimBuf.limit()) { // 当前匹配到字节个数与文本换行符字节个数相同，匹配结束
					...
					try {
						...
						logger.info("in CharsetDecoderII dodecode string is ==========================="
								+ dString);
					} finally {
						buf.clear(); // 释放缓存空间
					}
